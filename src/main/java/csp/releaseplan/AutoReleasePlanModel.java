...
public class AutoReleasePlanModel {
    ...
    IntVar[] fitnessReleasePlan_; //assignes releases to each requirement
    ...
    private void modelSoftconstraintOptimization(int noReleases, int noRequirements) {
        ...
        for (int i = 0; i < fitnessReleasePlan_.length; i++) {
            m.ifThenElse(m.arithm(releasePlan_[i], "=", 0),
                    m.arithm(fitnessReleasePlan_[i], "=", noReleases + 1),
                    m.arithm(fitnessReleasePlan_[i], "=", releasePlan_[i]));
            priorities_[i] = datasource_.getRequirementPriority(getRequirementId(i));
        }

        IntVar[] fitnessValues = m.intVarArray(noRequirements, 0, Arrays.stream(priorities_).max().getAsInt() * noReleases);
        objective_ = m.intVar("objective", 0, noRequirements * noReleases * noReleases);
        for (int requirement = 0; requirement < noRequirements; requirement++) {
    ...
    public void setObjective() {
        if (objective_ != null) {
            m.setObjective(Model.MINIMIZE, objective_);
    ...
    public void printCurrentSolution() {
        ...
        for (int i = 0; i < releasePlan_.length; i++) {
            System.out.println(releasePlan_[i].getValue() + "(" + fitnessReleasePlan_[i].getValue() + ")");
        }
